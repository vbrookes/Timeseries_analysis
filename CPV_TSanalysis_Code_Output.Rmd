---
title: "Demostration code: Parvovirus time-series analysis in R"
...
```{r}
fit <- auto.arima(Parvo_ts_dataM, xreg=Rainfall_ts_data)
summary(fit)
confint(fit)
checkresiduals(fit)

fcast <- forecast(fit, xreg=rep(mean(Rainfall_ts_data),24))

autoplot(fcast) + xlab("Year") + ylab("Events")
```

A increase in rainfall is associated with a decrease in parvo events, and vice versa. This model has a lower AICc (628.53) than the model without rainfall as a predictor (AICc = 632.61). The forecast plot results in a slightly narrower range of predictions over time than the model without rainfall as a predictor.
